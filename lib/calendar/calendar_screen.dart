...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Future<List<Map<String, dynamic>>> _getColaboradores() async {
    ...
    return snapshot.docs
        .map((doc) => {'id': doc.id, 'name': doc['name'], 'email': doc['email']})
        .toList();
  }
  ...
  Future<void> _guardarActividad({String? docId}) async {
    ...
    if (docId == null) {
      ...
      await FirebaseFirestore.instance.collection('actividades').doc(docId).update(data);
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _cargarActividadParaEditar(Map<String, dynamic> actividad, String docId) {
    setState(() {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(docId == null ? Icons.add_circle : Icons.edit, color: Colors.indigo, size: 28),
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: IconButton(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            List<Location> locations = await locationFromAddress(_direccionController.text.trim());
                            if (locations.isNotEmpty) {
                              ...
                              setStateDialog(() {
                                ...
                                _direccionManual = _direccionController.text.trim();
                              });
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Dirección encontrada y seleccionada')),
                              );
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: colaboradores
                        .map<DropdownMenuItem<String>>((col) => DropdownMenuItem<String>(
                              value: col['email'],
                    ...
                    children: [
                      Expanded(
                        child: _ubicacionLatLng == null
                            ? const Text('Sin ubicación seleccionada', style: TextStyle(fontSize: 16))
                            : Text(
                    ...
                    children: [
                      if (docId != null)
                        TextButton.icon(
                          ...
                          onPressed: () async {
                            await FirebaseFirestore.instance.collection('actividades').doc(docId).delete();
                      ...
                      const SizedBox(width: 8),
                      ElevatedButton.icon(
                        icon: Icon(docId == null ? Icons.save : Icons.edit, color: Colors.white),
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () => _cargarActividadParaEditar(actividad, docId),
                  ),
